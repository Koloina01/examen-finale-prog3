    ...
    public static Validation toEntity(ResultSet rs) {
        ...
        try {
            ...
            if (rs.getTimestamp("validated_at") != null) {
                v.setValidatedAt(rs.getTimestamp("validated_at").toLocalDateTime());
            }

            return v;

        } catch (SQLException e) {
            System.out.println("Erreur mapping Validation : " + e.getMessage());
            return null;
        }
    }
    public ValidationResponse toResponse(Validation v) {
        ...
        r.setId(v.getId());
        r.setOutcome(v.getOutcome());
        r.setComment(v.getComment());
        r.setValidatedAt(v.getValidatedAt());
